	...
	function (model, oldEvent, newEvent) {
		var newEvents = A2(
			...
			A2(
				...
				function (e) {
					return !_Utils_eq(e.id, oldEvent.id);
				},
				model.events),
			_List_fromArray(
				[newEvent]));
...
var $author$project$Main$setEventEditing = F3(
	function (model, event, field) {
		var newEvent = _Utils_update(
			event,
			{editing: field});
		return A3($author$project$Main$replaceEvent, model, event, newEvent);
	});
	...
	function (msg, model) {
		switch (msg.$) {
					...
					_Utils_update(
						model,
						{
							events: A2(
								$elm$core$List$filter,
								function (e) {
									return !_Utils_eq(e.id, eventToDelete.id);
								},
								model.events)
						}),
					$elm$core$Platform$Cmd$none);
				...
				var updatedEvents = A2(
					...
					_List_fromArray(
						[
							...
							event,
							{
								editing: $author$project$Types$Title(input)
							})
						]));
				return _Utils_Tuple2(
					_Utils_update(
			...
			case 'UserClickedTitle':
				...
				return _Utils_Tuple2(
					A3(
						$author$project$Main$setEventEditing,
						model,
						event,
						$author$project$Types$Title(event.title)),
					$elm$core$Platform$Cmd$none);
				...
				var updatedEvents = A2(
					...
					_List_fromArray(
						[
							...
							event,
							{
								editing: $author$project$Types$Duration(newDurationString)
							})
						]));
				var newDuration = function () {
	...
	var titleHtml = function () {
		...
		if (_v1.$ === 'Title') {
			...
			return A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$value(tmpTitle),
	...
	var durationHtml = function () {
		...
		if (_v0.$ === 'Duration') {
			...
			return A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$value(tmpDuration),
						$elm$html$Html$Events$onInput(
...
var $author$project$Date$getDow = function (date) {
	return A2(
...
var $author$project$Date$firstDateOfWeekZero = function (year) {
	...
	var _v0 = $author$project$Date$getDow(janFirst);
	switch (_v0.$) {
		case 'Mon':
			return janFirst;
		case 'Tue':
...
var $author$project$Date$fromMonth = function (month) {
	switch (month.$) {
		case 'Jan':
			return 'January';
		case 'Feb':
...
var $author$project$Date$format = function (_v0) {
	...
	var m = _v0.b;
	var d = _v0.c;
	var weekdayString = $author$project$Date$fromWeekday(
		$author$project$Date$getDow(
			A3($author$project$Date$Date, y, m, d)));
	var monthString = $author$project$Date$fromMonth(m);
	return A2(
		$elm$core$String$join,
		' ',
		_List_fromArray(
...
var $author$project$View$isEventOnThisDay = F2(
	function (date, event) {
...
var $author$project$View$nbMultipleDayEventsOnThisDay = F2(
...
var $author$project$View$nbSingleDayEventsOnThisDay = F2(
	function (events, date) {
		return $elm$core$List$length(
			A2(
				$elm$core$List$filter,
...
var $author$project$View$viewCalendarCell = F2(
	function (model, date) {
		var singleDayEventsOnThisDaysClass = (A2($author$project$View$nbSingleDayEventsOnThisDay, model.events, date) > 0) ? 'single-events' : '';
		var nextWeekDay = A2($author$project$Date$addDay, date, 7);
		var multipleDayEventsOnThisDaysClass = (A2($author$project$View$nbMultipleDayEventsOnThisDay, model.events, date) > 0) ? 'multi-events' : '';
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$Attributes$class(singleDayEventsOnThisDaysClass + (' ' + multipleDayEventsOnThisDaysClass))
						]),
					_List_fromArray(
						[
							$elm$html$Html$text(
...
var $author$project$View$viewWeek = F2(
	function (model, weekNumber) {
		...
		var m = _v0.b;
		var monthColContent = ((diffDays < 0) || ($author$project$Date$getDay(firstDateOfWeek) === 1)) ? _List_fromArray(
			[
				$elm$html$Html$text(
				$author$project$Date$fromMonth(m))
			]) : _List_Nil;
		return A2(
			$elm$html$Html$tr,
			_List_Nil,
